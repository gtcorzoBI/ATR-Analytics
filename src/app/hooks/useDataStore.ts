  ...
  return {
    ...
    approveDashboard: (pubId: string, areaId: string) => {
      ...
      const newDash = {
        id: `dash-${Date.now()}`,
        title: dash.name,
        category: AREA_NAMES[areaId],
        config: dash,
      };

      internal_system = {
        ...internal_system,
        [areaId]: [...(internal_system[areaId] || []), newDash],
      };
      ...
      persistBackend("/api/dev/system", "POST", {
        areaId,
        dashId: newDash.id,
        dashboard: newDash,
      });
      ...
      if (token) {
        try {
          await fetch(`${API}/api/dev/canvas`, {
            ...
            body: JSON.stringify({ id: "active_canvas", items }),
          });
        } catch (err) {
    ...
    resetCanvas: async () => {
      internal_canvas = [];
      localStorage.removeItem("atr_dev_canvas");
      localStorage.removeItem("atr_dev_snapshot");
      notify();

      const token = localStorage.getItem("atr_token");
      if (token) {
        try {
          await fetch(`${API}/api/dev/canvas/active_canvas`, {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          });
        } catch (err) {
          console.error("Failed to clear backend canvas", err);
        }
      }
    },
